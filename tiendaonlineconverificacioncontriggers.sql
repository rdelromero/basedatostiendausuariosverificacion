...
/*Tabla productos: precio_final de un producto al actualizar el producto*/
DELIMITER $$
...
/*Tabla productos: Recálculo de la valoración media de un producto al borrar una reseña*/
DELIMITER $$
CREATE TRIGGER actualizar_valoracion_media_after_delete
AFTER DELETE ON resenas
FOR EACH ROW
BEGIN
    -- Recalcula la valoración media y ajusta el número de valoraciones
    UPDATE productos
    SET numero_valoraciones = numero_valoraciones - 1,
        valoracion_media = CASE
            WHEN numero_valoraciones = 0 THEN NULL  -- No hay más reseñas después de la eliminación
            ELSE (valoracion_media * (numero_valoraciones + 1) - OLD.valoracion) / numero_valoraciones
        END
    WHERE id_producto = OLD.identidad_producto;
END$$
DELIMITER ;
...
/*Tabla linea_facturacion al actualizar: Calculo de precio_linea*/
DELIMITER $$
CREATE TRIGGER before_update_linea_facturacion
BEFORE UPDATE ON linea_facturacion
FOR EACH ROW
BEGIN
    -- Comprobar si el producto o la cantidad ha cambiado
    IF NEW.identidad_producto <> OLD.identidad_producto OR NEW.cantidad <> OLD.cantidad THEN
        -- Obtener el precio final del nuevo producto relacionado
        SELECT precio_final INTO @precio_unitario
        FROM productos
        WHERE id_producto = NEW.identidad_producto;

        -- Actualizar precio unitario y recalcular precio línea
        SET NEW.precio_unitario = @precio_unitario;
        SET NEW.precio_linea = NEW.precio_unitario * NEW.cantidad;
    END IF;
END$$
...
/*Tabla pedidos al actualizar un registro en la tabla linea_facturacion: recálculo de precio_total*/
DELIMITER $$
CREATE TRIGGER after_lineafacturacion_update
AFTER UPDATE ON linea_facturacion
FOR EACH ROW
BEGIN
    UPDATE pedidos
    SET precio_total = (SELECT SUM(precio_linea) FROM linea_facturacion WHERE identidad_pedido = NEW.identidad_pedido)
...
/*Tabla pedidos al borrar en la tabla linea_facturacion: recálculo de precio_total*/
DELIMITER $$
CREATE TRIGGER after_lineafacturacion_delete
AFTER DELETE ON linea_facturacion
FOR EACH ROW
BEGIN
    UPDATE pedidos
    SET precio_total = (SELECT SUM(precio_linea) FROM linea_facturacion WHERE identidad_pedido = OLD.identidad_pedido)
    WHERE id_pedido = OLD.identidad_pedido;
END$$
DELIMITER ;
